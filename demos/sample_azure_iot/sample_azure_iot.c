 ...
 */
#define sampleazureiotMESSAGE                                 "Hello! : %d"
...
static uint8_t ucPropertyBuffer[ 128 ];
...
static void prvAzureDemoTask( void * pvParameters )
{
	...
	for( ; ; )
	{
		if( xAzureSample_IsConnectedToInternet() )
		{
			...
			sprintf(tempBuf, "%d", _temp);
			sprintf(humiBuf, "%d", _humi);

			xResult = AzureIoTMessage_PropertiesAppend( &xPropertyBag, ( uint8_t * ) "Temperature", sizeof( "Temperature" ) - 1,
					( uint8_t * ) tempBuf, sizeof( tempBuf ));
			configASSERT( xResult == eAzureIoTSuccess );

			xResult = AzureIoTMessage_PropertiesAppend( &xPropertyBag, ( uint8_t * ) "Humidity", sizeof( "Humidity" ) - 1,
					( uint8_t * ) humiBuf, sizeof( humiBuf ));
			configASSERT( xResult == eAzureIoTSuccess );
